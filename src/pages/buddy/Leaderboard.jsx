...
const Leaderboard = () => {
  const { getLeaderboard, currentUser } = useAppStore();
      ...
      {leaderboard.length > 0 ? (
        <>
          ...
          {leaderboard.length >= 3 && (
            <div className="grid grid-cols-3 gap-3">
              {leaderboard.slice(0, 3).map((p, i) => (
                <motion.div key={p.uniqueId} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
                  ...
                  <p className={`text-xs font-semibold truncate ${p.isYou ? 'text-accent' : 'text-white'}`}>{p.name}</p>
          ...
          <div className="card space-y-2">
            {leaderboard.map((p, i) => (
              <motion.div key={p.uniqueId} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                ...
                <div className="flex-1">
                  <p className={`text-sm font-semibold ${p.isYou ? 'text-accent' : 'text-white'}`}>{p.name}</p>
        ...
        <div className="card text-center py-16 text-gray-500">
          ...
          <p className="text-xs mt-1">Complete quizzes to appear on the leaderboard. Your score is tracked from your unique ID.</p>
        </div>
      )}
    </motion.div>
  );
